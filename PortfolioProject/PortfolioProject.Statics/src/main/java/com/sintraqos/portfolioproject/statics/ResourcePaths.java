...
public class ResourcePaths {
    ...
    // OST_mus_a_xxx – These are event specific tracks. The ‘xx’ is an arbitrary number which is tied to events. For example, ‘mus_a_261’ plays when you crash into the Polar Area of Telos.
    // OST_mus_s_xxx – These are short (‘s’) tracks that reoccur occasionally.
    // The ‘xxx’ is the instance it’s tied to. Each of these are pretty self-explanatory, for example ‘mus_s_darkside’ plays when you get darkside points. ‘mus_s_romance’ plays during your character romance etc.
    ...
    public static final ArrayList<String> OST_AMBIENT_PREFIXES = new ArrayList<>() {
        {
            // Planets
            add("OST_amb_dan");     // All ambient sounds from the planet Dantooine
            add("OST_amb_dxn");     // All ambient sounds from the planet Dxun
            add("OST_amb_kor");     // All ambient sounds from the planet Korriban
            add("OST_amb_mal");     // All ambient sounds from the planet Malachor V
            add("OST_amb_nar");     // All ambient sounds from the planet Nar Shaddaa
            add("OST_amb_ond");     // All ambient sounds from the planet Onderon
            add("OST_amb_tel");     // All ambient sounds from the planet Telos

            // Ship
            add("OST_amb_ebo");     // All ambient sounds from the ship Ebon Hawk
            add("OST_amb_har");     // All ambient sounds from the ship Harbinger (Ship docking at Peragus mining facility)
            add("OST_amb_nih");     // All ambient sounds from the ship Ravager (Nihilus Capital Ship)

            // Other
            add("OST_amb_per");     // All ambient sounds from the Peragus mining facility
        }
    };
    ...
    public static final String OST_LIGHT_SIDE_SHORT = "OST_s_lighthort";        // Play this when character makes light side choices
    // Dark Side
